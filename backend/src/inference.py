...
def predict_lstm_sequence(rid: int):
    ...
    # Retrieve the sequence and the actual months visited
    ...
    # Convert the weights to a numpy array and remove batch dimensions
    attn_full = attn_weights.cpu().numpy().squeeze()
    
    # --- THE CRITICAL FIX: SLICE WEIGHTS TO MATCH ACTUAL VISITS ---
    # This prevents the "same size" issue and the "6 bars" issue by removing padding
    num_actual_visits = len(months)
    attn = attn_full[:num_actual_visits].tolist()
    
    # Ensure it handles single-visit patients as a list
    ...
    # Return the sliced attention and corresponding months
    return pred_class, prob_map, attn, list(map(int, months.tolist()))
